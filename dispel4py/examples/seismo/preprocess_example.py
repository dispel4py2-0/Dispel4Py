...
IN1 = (
    ROOT_DIR
    + "A25A.TA..BHZ.2011.025.00.00.00.000-2011.026.00.00.39.000.rm.scale-AUTO.SAC"
)
IN2 = (
    ROOT_DIR
    + "BMN.LB..BHZ.2011.025.00.00.00.023-2011.026.00.00.38.998.rm.scale-AUTO.SAC"
)
...
class StreamToFile(ConsumerPE):
    ...
    def _process(self, data):
        ...
        str1 = data[1]
        try:
            fout = self.file_dest.format(count)
        except TypeError:
            fout = file_dest
        str1.write(fout, format="SAC")
        self.log(f"Wrote file to {fout}")
...
def filter(str1, freqmin=0.01, freqmax=1.0, corners=4, zerophase=False):
    str1.filter(
        "bandpass",
        freqmin=freqmin,
        freqmax=freqmax,
...
streamToFile = StreamToFile(ROOT_DIR + "OUTPUT/NOWHITE/{}_preprocessed.SAC")
streamToFile.name = "StreamToFileNonWhitened"
streamToFileWhitened = StreamToFile(ROOT_DIR + "OUTPUT/WHITE/{}_preprocessed.SAC")
...
graph.connect(
    whiten,
    IterativePE.OUTPUT_NAME,
    streamToFileWhitened,
    StreamToFile.INPUT_NAME,
)
