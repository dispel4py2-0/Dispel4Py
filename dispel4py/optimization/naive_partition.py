...
def partition_all_pes(ct, pt):
    ...
    for i in range(len(ct)-1):
        ...
        if source not in [item for sublist in pe_partition.values() for item in sublist]:
            ...
            if partitions != []:
                ...
                if dest not in [item for sublist in pe_partition.values() for item in sublist]:
